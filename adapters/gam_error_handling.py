...
def map_gam_exception(exception: Exception) -> GAMError:
    ...
    if "AuthError" in type(exception).__name__ or "authentication" in error_message.lower():
        ...
        return GAMError(
            f"GAM error: {error_message}",
            GAMErrorType.UNKNOWN,
            error_details
        )
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            for attempt in range(retry_config.max_attempts):
                try:
                    ...
                    if attempt > 0:
                        logger.info(
                            f"Retrying {op_name} (attempt {attempt + 1}/{retry_config.max_attempts})"
                        )
                    ...
                    should_retry = (
                        gam_error.recoverable and 
                        any(isinstance(gam_error, exc_type) for exc_type in retry_on) and
    ...
    def rollback(self) -> List[Dict[str, Any]]:
        ...
        for step in reversed(self.steps):
            if step["rollback_action"]:
                try:
                    logger.info(
                        f"Rolling back {step['step_name']} for {step['resource_type']} "
                        f"{step['resource_id']}"
                    )
...
class GoogleAdManager(AdServerAdapter):
    ...
    def _create_gam_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        order_service = self.client.GetService('OrderService')
        created_orders = order_service.createOrders([order_data])
        
        # Validate response
        if not created_orders or len(created_orders) == 0:
            raise GAMValidationError("No order returned from createOrders")
            
        validate_gam_response(created_orders[0], ['id', 'name', 'status'])
        return created_orders[0]
    
    def create_media_buy(self, request: CreateMediaBuyRequest) -> CreateMediaBuyResponse:
        tracker = GAMOperationTracker(f"create_buy_{request.po_number}")
        
        try:
            # Create order with retry logic
            order = self._create_gam_order(order_data)
            tracker.add_step(
                "create_order",
                "Order",
                order['id'],
                rollback_action=lambda: self._archive_order(order['id'])
            )
            ...
            # Rollback on failure
            logger.error(f"Media buy creation failed: {e}")
            tracker.rollback()
            raise
"""