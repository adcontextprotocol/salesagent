...
from schemas import (
    ...
    Package, PackageStatus, CreateMediaBuyResponse
)
from config_loader import get_tenant_config
...
class XandrAdapter(AdServerAdapter):
    ...
    def _create_human_task(self, operation: str, details: Dict[str, Any]) -> str:
        ...
        """, (
            task_id,
            self.tenant_id,
            details.get('media_buy_id', 'N/A'),
            operation,
            f"Xandr: {operation.replace('_', ' ').title()}",
            json.dumps(details),
            'pending',
            json.dumps(details),
    ...
    def create_media_buy(self, request: MediaBuyRequest) -> Tuple[MediaBuy, CreateMediaBuyResponse]:
        ...
        try:
            ...
            io_data = {
                "insertion-order": {
                    ...
                    "budget_intervals": [{
                        "start_date": request.flight_start_date.isoformat(),
                        "end_date": request.flight_end_date.isoformat(),
                        "daily_budget": float(request.total_budget / (request.flight_end_date - request.flight_start_date).days),
            ...
            for package_req in request.packages:
                li_data = {
                    "line-item": {
                        ...
                        "daily_budget": float(package_req.budget / (request.flight_end_date - request.flight_start_date).days),
                        "currency": "USD",
            ...
            # Log the operation
            self._log_operation('create_media_buy', True, {
                'insertion_order_id': io_id,
                'line_item_count': len(packages)
            })
    ...
    def _create_targeting_profile(self, targeting: Dict[str, Any]) -> int:
        ...
        if 'geo' in targeting:
            geo = targeting['geo']
            if 'countries' in geo:
                profile_data["profile"]["country_targets"] = [
                    {"country": c} for c in geo['countries']
                ]
            if 'regions' in geo:
                profile_data["profile"]["region_targets"] = [
                    {"region": r} for r in geo['regions']
                ]
            if 'cities' in geo:
                profile_data["profile"]["city_targets"] = [
                    {"city": c} for c in geo['cities']
                ]
        ...
        if 'device_types' in targeting:
            ...
            profile_data["profile"]["device_type_targets"] = [
                device_map.get(d, 1) for d in targeting['device_types']
            ]
    ...
    def update_media_buy(self, media_buy_id: str, updates: MediaBuyDetails) -> MediaBuy:
        ...
        if self._requires_manual_approval('update_media_buy'):
            task_id = self._create_human_task('update_media_buy', {
    ...
    def get_media_buy_status(self, media_buy_id: str) -> MediaBuyStatus:
        ...
        try:
            ...
            spent = sum(li.get('lifetime_budget_imps', 0) * li.get('revenue_value', 0) / 1000 
            ...
            for li in line_items:
                package_statuses.append(
                    PackageStatus(
                        ...
                        delivery_percentage=(li.get('lifetime_budget_imps', 0) / 
                                           li.get('lifetime_pacing', 1)) * 100 if li.get('lifetime_pacing') else 0
                    )
                )
        ...
        try:
            ...
            report_data = {
                "report": {
                    ...
                    "columns": [
                        "hour", "imps", "clicks", "media_cost", "booked_revenue",
                        "line_item_id", "line_item_name", "creative_id", "creative_name"
                    ],
                    "filters": [
                        {"insertion_order_id": int(io_id)}
                    ],
    ...
    def pause_media_buy(self, media_buy_id: str) -> bool:
        ...
        try:
            ...
            for li in li_response['response']['line-items']:
                self._make_request('PUT', f'/line-item?id={li["id"]}', {
                    "line-item": {"state": "inactive"}
                })
    ...
    def update_package(self, media_buy_id: str, packages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self._requires_manual_approval('update_package'):
            task_id = self._create_human_task('update_package', {
                'media_buy_id': media_buy_id,
                'packages': packages,
                'principal': self.principal.name
            })
            ...
            return {
                "status": "accepted",
                "task_id": task_id,
                "detail": "Package updates require manual approval"
            }
        ...
        try:
            ...
            report_data = {
                "report": {
                    ...
                    "columns": [
                        "day", "insertion_order_id", "insertion_order_name",
                        "line_item_id", "line_item_name", "creative_id", "creative_name",
                        "imps", "clicks", "media_cost", "booked_revenue",
            ...
            # Calculate metrics
            summary["ctr"] = (summary["total_clicks"] / summary["total_impressions"]) if summary["total_impressions"] > 0 else 0
            summary["cpm"] = (summary["total_spend"] / summary["total_impressions"] * 1000) if summary["total_impressions"] > 0 else 0
            summary["completion_rate"] = (summary["video_completions"] / summary["video_starts"]) if summary["video_starts"] > 0 else 0
    ...
    def get_creative_performance(self, media_buy_id: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            report_data = {
                "report": {
                    ...
                    "columns": [
                        "creative_id", "creative_name", "line_item_id", "line_item_name",
                        "imps", "clicks", "media_cost", "video_starts", "video_completions",
                        "viewability_measurement_impressions", "viewability_viewed_impressions"
                    ],
                    "filters": [
                        {"insertion_order_id": int(io_id)}
                    ],
