...
class GAMImplementationConfig(BaseModel):
    ...
    applied_team_ids: List[int] = Field(
        default_factory=list,
        description="GAM team IDs for access control"
    )
    ...
    creative_rotation_type: str = Field(
        "EVEN",
        description="How to rotate creatives: EVEN, OPTIMIZED, MANUAL, SEQUENTIAL"
    )
    delivery_rate_type: str = Field(
        "EVENLY",
        description="Delivery pacing: EVENLY, FRONTLOADED, AS_FAST_AS_POSSIBLE"
    )
    
    # Pricing and goals
    cost_type: str = Field(
        "CPM",
        description="Pricing model: CPM, CPC, CPD, CPA"
    )
    discount_type: Optional[str] = Field(
        None,
        description="Discount type if applicable: PERCENTAGE, ABSOLUTE_VALUE"
    )
    discount_value: Optional[float] = Field(
        None,
        description="Discount value (percentage or absolute based on discount_type)"
    )
    
    primary_goal_type: str = Field(
        "LIFETIME",
        description="Goal type: LIFETIME, DAILY, NONE"
    )
    primary_goal_unit_type: str = Field(
        "IMPRESSIONS",
        description="Goal unit: IMPRESSIONS, CLICKS, VIEWABLE_IMPRESSIONS"
    )
    ...
    targeted_ad_unit_ids: List[str] = Field(
        default_factory=list,
        description="Specific GAM ad unit IDs to target"
    )
    excluded_ad_unit_ids: List[str] = Field(
        default_factory=list,
        description="GAM ad unit IDs to exclude"
    )
    targeted_placement_ids: List[str] = Field(
        default_factory=list,
        description="GAM placement IDs to target"
    )
    include_descendants: bool = Field(
        True,
        description="Include child ad units in targeting"
    )
    
    # Frequency capping
    frequency_caps: List[FrequencyCap] = Field(
        default_factory=list,
    ...
    competitive_exclusion_labels: List[str] = Field(
        default_factory=list,
        description="Labels to prevent competitive ads from serving together"
    )
    
    # Video-specific settings (optional)
    environment_type: str = Field(
        "BROWSER",
        description="Environment: BROWSER or VIDEO_PLAYER"
    )
    companion_delivery_option: Optional[str] = Field(
        None,
        description="For video: OPTIONAL, AT_LEAST_ONE, ALL"
    )
    video_max_duration: Optional[int] = Field(
        None,
        description="Maximum video duration in milliseconds"
    )
    skip_offset: Optional[int] = Field(
        None,
        description="When skip button appears (milliseconds from start)"
    )
    
    # Advanced settings
    disable_viewability_avg_revenue_optimization: bool = Field(
        False,
        description="Disable viewability-based optimization"
    )
    allow_overbook: bool = Field(
        False,
        description="Allow overbooking of inventory"
    )
    skip_inventory_check: bool = Field(
        False,
        description="Skip inventory availability check"
    )
    
    # Custom targeting template
    custom_targeting_keys: Dict[str, Any] = Field(
        default_factory=dict,
        description="Custom key-value pairs for targeting"
    )
    
    # Native ad settings
    native_style_id: Optional[str] = Field(
        None,
...
EXAMPLE_VIDEO_CONFIG = {
    ...
    "primary_goal_unit_type": "IMPRESSIONS",
    "creative_placeholders": [
        {"width": 640, "height": 480, "expected_creative_count": 1}
    ],
