...
class GAMLogContext:
    ...
    def _summarize_request(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe_fields = [
            "name", "orderId", "lineItemId", "advertiserId",
            "status", "priority", "targeting", "budget"
        ]
...
def log_gam_operation(
    ...
    try:
        ...
        if context.success:
            logger.info(
                f"GAM operation completed: {operation.value}",
                extra=log_data
            )
...
def _send_metrics(context: GAMLogContext):
    ...
    if _send_metrics.call_count % 100 == 0:
        logger.info(
            "GAM metrics summary",
            extra={"metrics": _metrics.get_metrics()}
        )
...
def _store_audit_log(context: GAMLogContext):
    ...
    audit_data = {
        ...
        "duration_ms": (context.end_time - context.start_time) * 1000 if context.start_time and context.end_time else None,
        "error": str(context.error) if context.error else None
    }
    
    # This would integrate with the existing audit_logger
    logger.info(
        f"AUDIT: GAM {context.operation.value}",
        extra=audit_data
    )
...
def log_dry_run(operation: str, details: Dict[str, Any]):
    ...
    logger.info(
        f"[DRY RUN] Would execute: {operation}",
        extra={
            "dry_run": True,
            "operation": operation,
            "details": details
...
def log_configuration(config_type: str, config: Dict[str, Any]):
    ...
    safe_config = {
        ...
        if k not in ["service_account_key_file", "access_token", "api_key"]
    }
    
    logger.info(
        f"GAM configuration loaded: {config_type}",
        extra={
