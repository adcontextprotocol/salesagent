...
def upgrade():
    ...
    # Check if table already exists (for test compatibility)
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    existing_tables = inspector.get_table_names()
    
    if 'contexts' not in existing_tables:
        op.create_table('contexts',
            ...
            sa.Column('conversation_history', sa.JSON, nullable=False, server_default='[]'),  # Clarifications and refinements only
    ...
    if 'workflow_steps' not in existing_tables:
        op.create_table('workflow_steps',
            ...
            sa.Column('status', sa.String(20), nullable=False, server_default='pending'),  # pending, in_progress, completed, failed, requires_approval
    ...
    if 'tasks' in existing_tables:
        tasks_cols = [col['name'] for col in inspector.get_columns('tasks')]
        if 'context_id' not in tasks_cols:
            op.add_column('tasks', sa.Column('context_id', sa.String(100), nullable=True))
            if 'contexts' in existing_tables:
                op.create_foreign_key('fk_tasks_context', 'tasks', 'contexts',
                                     ['context_id'], ['context_id'], ondelete='SET NULL')
            op.create_index('idx_tasks_context', 'tasks', ['context_id'])
        
        # Add fields to tasks table for better integration
        if 'human_needed' not in tasks_cols:
            op.add_column('tasks', sa.Column('human_needed', sa.Boolean, nullable=False, server_default='0'))
        if 'message' not in tasks_cols:
            op.add_column('tasks', sa.Column('message', sa.Text, nullable=True))
        if 'clarification_needed' not in tasks_cols:
            op.add_column('tasks', sa.Column('clarification_needed', sa.Boolean, nullable=False, server_default='0'))
        if 'clarification_details' not in tasks_cols:
