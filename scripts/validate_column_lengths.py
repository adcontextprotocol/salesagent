...
def extract_string_length(node: ast.AST) -> Optional[int]:
        ...
        for class_name, columns in model_columns.items():
            for col_name, length in columns.items():
                if col_name in COLUMN_LENGTHS:
                    ...
                    if length != expected:
                        issues.append(
                            f"{models_path}: {class_name}.{col_name} has length {length}, "
                            f"expected {expected}"
                        )
                    ...
                    try:
                        ...
                        if match:
                            ...
                            if found_length < min_length:
                                issues.append(
                                    f"{filepath}: {col_name} length {found_length} < {min_length} "
                                    f"({reason})"
                                )
                    except:
                        pass
