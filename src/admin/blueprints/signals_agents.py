...
def list_signals_agents(tenant_id):
    ...
    try:
        with get_db_session() as session:
            ...
            stmt = select(SignalsAgent).filter_by(tenant_id=tenant_id).order_by(SignalsAgent.priority)
                ...
                agents_list.append(
                    {
                        ...
                        "fallback_to_database": agent.fallback_to_database,
                        "timeout": agent.timeout,
                        "max_signal_products": agent.max_signal_products,
...
def add_signals_agent(tenant_id):
    ...
    try:
        with get_db_session() as session:
            ...
            fallback_to_database = request.form.get("fallback_to_database") == "on"
            timeout = int(request.form.get("timeout", "30"))
            max_signal_products = int(request.form.get("max_signal_products", "10"))
            ...
            agent = SignalsAgent(
                ...
                timeout=timeout,
                max_signal_products=max_signal_products,
            )
            session.add(agent)
            session.commit()
...
def edit_signals_agent(tenant_id, agent_id):
    ...
    try:
        with get_db_session() as session:
            ...
            agent.fallback_to_database = request.form.get("fallback_to_database") == "on"
            agent.timeout = int(request.form.get("timeout", "30"))
            agent.max_signal_products = int(request.form.get("max_signal_products", "10"))
