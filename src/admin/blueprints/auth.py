...
def google_auth():
    ...
    logger.info(
        ...
        f"tenant_context: {tenant_context}, external_domain: {approximated_host}"
    )

    # Let Authlib manage the state parameter for CSRF protection
    # Custom data is stored in session (above) and will be available in callback
    # Note: Session cookies work across subdomains due to Domain=.sales-agent.scope3.com
...
def google_callback():
    ...
    try:
        ...
        # Check if this is a signup flow (from session)
        ...
        # Session cookies work across subdomains due to Domain=.sales-agent.scope3.com
