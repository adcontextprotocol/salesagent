...
# CreativeAgentRegistry. This module used to re-derive it locally; three copies
# of a security gate mean a fix to one is a fix to one (CLAUDE.md DRY
# invariant). The helper reaches the signing layer through the
# ``src.core.signing`` FACADE, never a submodule
# (tests/unit/test_architecture_signing_layer_boundary.py rule B, allowlist
# permanently empty).
