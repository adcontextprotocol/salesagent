...
class SignalsAgentRegistry:
            ...
            for db_agent in db_agents:
                ...
                agents.append(
                    SignalsAgent(
                        ...
                        fallback_to_database=db_agent.fallback_to_database,
                        timeout=db_agent.timeout,
                        max_signal_products=db_agent.max_signal_products,
                    )
                )
    ...
    async def _get_signals_from_agent(
        ...
        # Normalize agent URL to avoid double slashes
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    raise RuntimeError(
                        f"Failed to connect to signals agent after {max_retries} attempts: {type(e).__name__}: {e}"
                    ) from last_exception
