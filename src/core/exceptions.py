...
# (enums/error-code.json @ v3.1.1, adcp==6.6.0) defines these as real wire codes;
# the SDK's ``STANDARD_ERROR_CODES`` predates them, and the SDK is a cross-check,
# not the authority. CREATIVE_NOT_FOUND per the enum: "Sellers MUST return this
# code uniformly for any creative_id not owned by the calling account" (#1430
# review). CONFIGURATION_ERROR per the enum: "the buyer cannot resolve a
# seller-side deployment misconfiguration and MUST NOT auto-retry" (#1430 review).
# BILLING_NOT_SUPPORTED and CREDENTIAL_IN_ARGS are likewise pinned wire codes
# absent from the SDK table, promoted here so the buyer sees the spec code
# untranslated (#1329). BILLING_NOT_SUPPORTED was previously demoted to
# UNSUPPORTED_FEATURE via ERROR_CODE_MAPPING, but its only emitter is the advisory
# ``errors[]`` path (build_advisory_error), which serializes verbatim and never hit
# that mapping — so the wire code was already BILLING_NOT_SUPPORTED; promoting it +
# dropping the mapping makes that honest (resolves the #1602 demotion).
# CREDENTIAL_IN_ARGS is the code for a buyer-principal credential detected in
# request args (authentication.mdx L2). Recovery classifications are NOT repeated
# here — this is a set of code NAMES; RECOVERY_BY_WIRE_CODE (the pinned
# enumMetadata) answers what they mean.
