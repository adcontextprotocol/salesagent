...
:func:`src.core.signing.webhook_sender_factory.webhook_delivery_signer`, the one
place that decides which key signs a tenant's webhooks.
...
def _headers_for(
    auth: LibraryAuthentication | None,
    ...
    * **No ``authentication`` block at all** -> the RFC 9421 arm. Absence is the
    ...
    if auth is None and signer is not None:
    ...
    if auth is None:
        ...
        return prepared, None, None if signer is None else signer.build_auth_headers
...
def deliver_webhook(
    ...
    *,
    ...
    :func:`src.core.signing.webhook_sender_factory.webhook_delivery_signer` — the ONE
    place that decides which key signs this tenant's webhooks, shared with the SDK
    sender the three services use today. It is APPLIED only on the arm the spec
    selects for it (no ``authentication`` block; see :func:`_headers_for`), and only
