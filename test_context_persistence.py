...
from models import Context, WorkflowStep
...
def test_simplified_context():
    ...
    console.print(f"  - Pending: {status['pending_count']}")
    console.print(f"  - In progress: {status['in_progress_count']}")
    ...
    ctx_manager.add_message(
        ctx.context_id,
        role="assistant",
        content="Media buy has been approved and created successfully."
    )
    console.print(f"✓ Added message to conversation")
    ...
    # Test 8: Verify no status/session_type fields
    console.print("\n[yellow]Test 8: Verifying simplified schema[/yellow]")
    ...
    console.print("  - No status field")
    console.print("  - No session_type field")
    console.print("  - No expires_at field")
    console.print("  - No task-related fields")
    
    console.print("\n[bold green]All tests passed![/bold green]")
    console.print("\n[cyan]Key takeaways:[/cyan]")
    console.print("1. Synchronous operations don't need context at all")
    console.print("2. Context is just for conversation tracking in async operations")
    console.print("3. Workflow steps table is the actual work queue")
    console.print("4. Status comes from workflow steps, not from context")
