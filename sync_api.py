...
from models import SyncJob, Tenant, AdapterConfig
from gam_inventory_service import GAMInventoryService, db_session as gam_db_session
...
def get_superadmin_api_key() -> Optional[str]:
...
def trigger_sync(tenant_id: str):
    ...
    try:
        ...
        if not force:
            recent_sync = db_session.query(SyncJob).filter(
                SyncJob.tenant_id == tenant_id,
                SyncJob.status.in_(['running', 'completed']),
                SyncJob.started_at >= datetime.now(timezone.utc).replace(hour=0, minute=0, second=0)
            ).first()
...
def list_tenants():
    ...
    try:
        ...
        tenants = db_session.query(Tenant).filter_by(
        ...
        for tenant in tenants:
            ...
            last_sync = db_session.query(SyncJob).filter(
                SyncJob.tenant_id == tenant.tenant_id,
...
def get_sync_stats():
    ...
    try:
        ...
        for status in ['pending', 'running', 'completed', 'failed']:
            count = db_session.query(SyncJob).filter(
        ...
        # Get recent failures
        recent_failures = db_session.query(SyncJob).filter(
            SyncJob.status == 'failed',
            SyncJob.started_at >= since
        ).order_by(SyncJob.started_at.desc()).limit(5).all()
        ...
        all_gam_tenants = db_session.query(Tenant).filter_by(
            ad_server='google_ad_manager'
        ).all()
        ...
        for tenant in all_gam_tenants:
            ...
            if not last_sync or (datetime.now(timezone.utc) - last_sync.completed_at).days > 1:
...
def initialize_superadmin_api_key() -> str:
    ...
    """, (
        'api_key',
        api_key,
        'Superadmin API key for programmatic access',
        'system'
    ))
