...
- `submitted` - Awaiting external approval (e.g., creative review)
- `canceled` - Operation was canceled

### Webhook Status Sequence

Protocol-level webhooks fire for **all status transitions** when `pushNotificationConfig` is provided (per AdCP PR #268):

#### Successful Operation Flow
```
1. "working" → Task starts processing
2. "completed" → Operation succeeded
```

#### Failed Operation Flow
```
1. "working" → Task starts processing
2. "failed" → Operation failed with error
```

#### Submitted Operation Flow (requires approval)
```
1. "working" → Task starts processing
2. "submitted" → Awaiting external approval (e.g., creative review)
```

#### Canceled Operation Flow
```
1. "working" → Task starts processing
2. "canceled" → Operation was canceled by client
```

**Example webhook sequence for create_media_buy:**
...
**Important:**
- Clients should handle all status updates, not just terminal states
- The "working" status arrives within milliseconds of task creation
- Webhook failures do not affect task processing (fire-and-forget model)
- The "working" webhook uses non-blocking dispatch to avoid delaying task responses
