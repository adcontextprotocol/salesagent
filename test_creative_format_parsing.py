...
def compare_format(actual: Dict[str, Any], expected: Dict[str, Any]) -> List[str]:
    ...
    if expected.get('description'):
        ...
        elif expected['description'].lower() not in actual['description'].lower() and \
             actual['description'].lower() not in expected['description'].lower():
        ...
        try:
            ...
            for i, expected_format in enumerate(expected_formats):
                if i < len(actual_formats):
                    ...
                    results['errors'].append(
                        f"{html_file.name}: Missing format {i+1} ({expected_format['name']})"
                    )
                    results['success'] = False
