...
**🚨 MANDATORY**: All test environments use `scripts/test_ports.py` for consistent port management.

**The Problem This Solves:**
- ❌ Hardcoded ports scattered across test files (5433, 5435, 8092, etc.)
- ❌ Conductor workspace-specific ports conflicting with CI (5496 vs 5433)
- ❌ Docker Compose using different ports than tests
- ❌ Configuration drift causing flaky tests and port conflicts

**The Solution: Single Source of Truth**

All ports come from `scripts/test_ports.py`:

```python
from scripts.test_ports import get_ports

ports = get_ports(mode="ci")  # or "e2e", "local", "auto"
...
1. **CI Mode** (`mode="ci"`) - GitHub Actions / CI environments:
   - postgres: 5433 (integration tests container)
   - mcp: 8092 (E2E MCP server)
   - a2a: 8094 (E2E A2A server)
   - admin: 8093 (E2E Admin UI)

2. **E2E Mode** (`mode="e2e"`) - Docker Compose E2E tests:
   - postgres: 5435 (inside docker-compose)
   - mcp: 8092 (maps to container 8080)
   - a2a: 8094 (maps to container 8091)
   - admin: 8093 (maps to container 8001)

3. **Local Mode** (`mode="local"`) - Conductor workspace:
   - Reads from .env file (CONDUCTOR_POSTGRES_PORT, CONDUCTOR_MCP_PORT, etc.)
   - Each workspace gets unique ports (e.g., 5496, 8144, 8155)

4. **Auto Mode** (`mode="auto"`) - Automatic detection (default):
   - CI environment → uses CI ports
   - Conductor workspace → uses .env ports
   - Fallback → uses CI ports

**Usage in Code:**

```bash
# run_all_tests.sh - Uses centralized ports
...
✅ **Single source of truth** - No more scattered hardcoded ports
✅ **Workspace isolation** - Conductor workspaces don't conflict
✅ **CI compatibility** - Matches GitHub Actions exactly
✅ **Type-safe** - TypedDict provides autocomplete and validation
✅ **Testable** - Port conflicts detected before tests run

**When Adding New Tests:**

1. Import and use `get_ports()`:
   ```python
   from scripts.test_ports import get_ports
   ports = get_ports()  # Auto-detect mode
   db_url = f"postgresql://...@localhost:{ports['postgres']}/..."
   ```

2. **Never hardcode ports** (5433, 8092, etc.) in test files
3. Let `test_ports.py` manage all port allocation
4. Tests work in CI, local, and Conductor workspaces automatically
...
if errors:
    print("Port issues:", errors)
```

**Status:**
- ✅ `run_all_tests.sh` uses centralized ports
- ✅ `tests/integration/conftest.py` uses centralized ports
- ✅ `tests/e2e/conftest.py` uses centralized ports
