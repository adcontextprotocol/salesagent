...
if [ "$1" = "verbose" ]; then
    ...
    pytest tests/unit/test_admin_ui_oauth.py -v -s
elif [ "$1" = "coverage" ]; then
    echo "Running with coverage report..."
    pytest tests/unit/test_admin_ui_oauth.py --cov=admin_ui --cov-report=html --cov-report=term
elif [ "$1" = "quick" ]; then
    echo "Running quick test summary..."
    pytest tests/unit/test_admin_ui_oauth.py -q
else
