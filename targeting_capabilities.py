...
TARGETING_CAPABILITIES: Dict[str, TargetingCapability] = {
    ...
    "geo_region": TargetingCapability(
        dimension="geo_region",
        access="overlay",
        description="State/province targeting"
    ),
    "geo_metro": TargetingCapability(
        dimension="geo_metro",
        access="overlay",
        description="Metro/DMA targeting"
    ),
    "geo_city": TargetingCapability(
        dimension="geo_city",
        access="overlay",
        description="City-level targeting"
    ),
    "geo_zip": TargetingCapability(
        dimension="geo_zip",
        access="overlay",
        description="Postal code targeting"
    ),
    ...
    "device_make": TargetingCapability(
        ...
        access="overlay",
        description="Device manufacturer targeting"
    ),
    "os": TargetingCapability(
        dimension="os",
        access="overlay",
        description="Operating system targeting"
    ),
    "browser": TargetingCapability(
        dimension="browser",
        access="overlay",
    ...
    "content_language": TargetingCapability(
        ...
        access="overlay",
        description="Content language targeting"
    ),
    "content_rating": TargetingCapability(
        dimension="content_rating",
        access="overlay",
        description="Content rating targeting"
    ),
    ...
    "audience_segment": TargetingCapability(
        ...
        access="overlay",
        description="Third-party audience segments"
    ),
    
    # Time targeting - available for overlay
    "daypart": TargetingCapability(
        dimension="daypart",
        access="overlay",
        description="Day and hour targeting"
    ),
    ...
    "frequency_cap": TargetingCapability(
        ...
        access="overlay",
        description="Impression frequency limits"
    ),
    ...
    "aee_segment": TargetingCapability(
        dimension="aee_segment",
        access="managed_only",
        description="AEE-computed audience segments",
        aee_signal=True
    ),
    "aee_score": TargetingCapability(
        dimension="aee_score",
        access="managed_only",
        description="AEE effectiveness scores",
        aee_signal=True
    ),
    "aee_context": TargetingCapability(
        dimension="aee_context",
        access="managed_only",
        description="AEE contextual signals",
        aee_signal=True
    ),
    
    # Platform-specific - both overlay and managed
    "custom": TargetingCapability(
        dimension="custom",
        access="both",
        description="Platform-specific custom targeting"
    )
}
...
def get_overlay_dimensions() -> List[str]:
    ...
    return [
        name for name, cap in TARGETING_CAPABILITIES.items()
        if cap.access in ["overlay", "both"]
    ]
