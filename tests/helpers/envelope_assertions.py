    ...
    *,
    ...
    Args:
        ...
        field: If provided, both ``adcp_error.field`` and ``errors[0].field``
                must EXACTLY equal this JSONPath-lite path into the buyer's
                request payload (``core/error.json`` @3.1.1 — e.g.
                ...
                interop bugs". Prefer this over ``field_substr``: a substring
                token like ``accounts`` or ``governance_agents`` is a prefix of
                several governance field paths, so it stays green on a field
                wrong for the scenario (#1329). ``None`` (the default) does not
                assert absence: ``field`` is optional in the schema, so most
                envelopes legally carry none. A call site that needs "no
                ``field`` key at all" asserts that itself.
