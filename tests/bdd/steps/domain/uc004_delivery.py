...
def then_hmac_computation(ctx: dict) -> None:
    ...
    Recomputes over the wire body (``request.body``), not a fresh ``json.dumps`` of the
    parsed payload — a recompute from the dict can silently agree with a sender that
    signed one serialization and transmitted another, which is exactly the defect
    ...
    request = _last_webhook_delivery(ctx)

    # Case-insensitive on the wire (RFC 9110), and the legacy X-Webhook-* spellings are
    # accepted as a fallback so a sender that has not moved to X-ADCP-* is graded rather
    # than reported as unsigned.
...
def then_skip_no_webhook(ctx: dict, mb_id: str) -> None:
    ...
    posted_mb_ids = [_sent_payload(delivery).get("media_buy_id") for delivery in _webhook_deliveries(ctx)]
...
def _assert_wire_rejection(ctx: dict, field: str) -> None:
    ...
    if isinstance(envelope, dict) and "adcp_error" in envelope:
        ...
        # AUTH_MISSING/AUTH_INVALID (the v3.1.1 split) replace the deprecated
        # AUTH_REQUIRED alias for auth failures — excluding only the literal
        # "AUTH_REQUIRED" string let an auth failure (e.g.
        # resolve_principal_or_raise's "principal not found" -> AUTH_INVALID)
        # masquerade as a legitimate client field rejection once the split landed.
