...
The dispatch's typed payload is read via require_payload/payload_or_none
(#1802 removed the ctx["response"] copy); ctx["error"] is any exception raised.
"""
...
def then_brandless_rejected_validation_error(ctx: dict) -> None:
    ...
    if error_envelope_or_none(ctx) is not None:
        # Seller's own validation (IMPL/A2A/REST) → assert the two-layer AdCP envelope.
        # Selector is #1802's ctx-side accessor (reads through the TransportResult,
        # not a stale ctx key); the assertion is the single sanctioned wire-error
        # surface, whose recovery defaults from the PINNED enum (pin-wins).
