...
class CircuitBreakerEnv(CircuitBreakerMixin, BaseTestEnv):
    ...
    def make_webhook_config(
        ...
        (salesagent-47n9.24, GH #1894). A MagicMock answers every attribute, so
        leaving that column set would let this mock keep feeding a column production
        no longer reads -- the failure mode a mock-based harness is worst at
        surfacing.
