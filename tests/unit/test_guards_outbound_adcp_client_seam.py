...
#: The RFC 9421 request-signing strategy, and every module that re-exports it.
SIGNER_CALLEES = frozenset({"RequestSignerStrategy"})
SIGNER_MODULES = frozenset({"src.core.signing", "src.core.signing.request_signer"})
...
def test_detector_flags_a_second_signer_builder(tmp_path):
    ...
    bad_file = tmp_path / "some_new_registry.py"
    bad_file.write_text(
        "from src.core.signing import RequestSignerStrategy as _Strategy\n\n"
