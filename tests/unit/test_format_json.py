...
def test_formats_json():
    ...
    for fmt in data.get('foundational_formats', []):
        ...
        if 'assets' in fmt:
            ...
            print(f"  ✗ Missing assets field!")
        
        # Check for legacy specs field
        if 'specs' in fmt:
            print(f"  ✓ Has legacy specs field (for backward compatibility)")
        else:
            print(f"  ! No legacy specs field")
    
    # Verify specific formats have expected assets
    print("\n\nVerifying specific format structures:")
    
    # Check video format
    video_format = next((f for f in data['foundational_formats'] if f['format_id'] == 'foundation_universal_video'), None)
    if video_format:
    ...
    carousel_format = next((f for f in data['foundational_formats'] if f['format_id'] == 'foundation_product_showcase_carousel'), None)
    if carousel_format:
