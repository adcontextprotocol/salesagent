...
def test_upgrade_legacy_string_format():
    ...
    assert str(result.agent_url) == DEFAULT_AGENT_URL  # AnyUrl object, convert to string for comparison
...
def test_upgrade_format_id_object_passthrough():
    ...
    assert str(result.agent_url) == "https://custom.example.com"  # AnyUrl object, convert to string
...
def test_upgrade_dict_without_agent_url():
    ...
    assert isinstance(result, FormatId)
    assert result.id == "display_300x250"
    assert str(result.agent_url) == DEFAULT_AGENT_URL  # AnyUrl object, convert to string
