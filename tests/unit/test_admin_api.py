...
def mock_db():
    ...
    with patch('superadmin_api.get_db_connection') as mock_conn:
        mock_db = MagicMock()
...
class TestSuperAdminAPI:
    ...
    def test_health_check_with_invalid_api_key(self, client, mock_db):
        """Test health check fails with invalid API key."""
        # Mock the API key query - returns a different valid key (not matching the one provided)
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = ('sk-valid-but-different-key',)
    ...
    def test_health_check_with_valid_api_key(self, client, mock_db, api_key):
        """Test health check succeeds with valid API key."""
        # Mock the API key query - returns just the config_value
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = (api_key,)  # Just config_value as a tuple
        mock_db.execute.return_value = mock_cursor
    ...
    def test_list_tenants(self, client, mock_db, api_key):
        ...
        mock_db.execute.side_effect = [mock_cursor_auth, mock_cursor_tenants]
    ...
    def test_create_tenant_missing_fields(self, client, mock_db, api_key):
        """Test creating tenant with missing required fields."""
        # Mock the API key query
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = (api_key,)  # Just config_value
        mock_db.execute.return_value = mock_cursor
    ...
    def test_create_tenant_success(self, client, mock_db, api_key):
        ...
        mock_db.connection.commit = MagicMock()
    ...
    def test_get_tenant_not_found(self, client, mock_db, api_key):
        ...
        mock_db.execute.side_effect = [mock_cursor_auth, mock_cursor_tenant]
    ...
    def test_update_tenant(self, client, mock_db, api_key):
        ...
        # Mock tenant exists check
        mock_cursor_check = MagicMock()
        mock_cursor_check.fetchone.return_value = ('tenant_123', 'Test Publisher', 'test', 1, 'standard', 'google_ad_manager', '2025-01-06T12:00:00')
        ...
        mock_db.execute.side_effect = [mock_cursor_auth, mock_cursor_check, mock_cursor_update, mock_cursor_select]
        mock_db.connection = MagicMock()
        mock_db.connection.commit = MagicMock()
    ...
    def test_delete_tenant_soft(self, client, mock_db, api_key):
        ...
        mock_db.execute.side_effect = [mock_cursor_auth, mock_cursor_check, mock_cursor_update]
        mock_db.connection = MagicMock()
        mock_db.connection.commit = MagicMock()
    ...
    def test_delete_tenant_hard(self, client, mock_db, api_key):
    ...
    def test_init_api_key_success(self, client, mock_db):
        ...
        mock_db.execute.side_effect = [mock_cursor_check, mock_cursor_insert]
        mock_db.connection = MagicMock()
        mock_db.connection.commit = MagicMock()
    ...
    def test_init_api_key_already_exists(self, client, mock_db):
        """Test initializing API key when one already exists."""
        # Mock existing API key
        mock_cursor = MagicMock()
        mock_cursor.fetchone.return_value = ('sk-existing-key',)
        mock_db.execute.return_value = mock_cursor
