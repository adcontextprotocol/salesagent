...
from database_session import get_db_session, DatabaseManager, get_or_404, get_or_create
from json_validators import (
    CommentModel, PlatformMappingModel, CreativeFormatModel,
    ensure_json_array, ensure_json_object, validate_json_schema
)
from models import Base, Tenant, Product, Principal, WorkflowStep, Context
...
class TestJSONValidation:
    ...
    def test_creative_format_validation(self):
        ...
        with pytest.raises(ValueError, match="String should match pattern"):
            CreativeFormatModel(
                format_id='test',
                name='Test',
                type='invalid',  # Not in allowed values
                description='Test'
            )
    ...
    def test_principal_platform_mappings(self, test_db):
        ...
        with get_db_session() as session:
            ...
            # Retrieve and verify
            retrieved = session.query(Principal).filter_by(
                principal_id='test_principal'
            ).first()
    ...
    def test_workflow_step_comments(self, test_db):
        ...
        with get_db_session() as session:
            ...
            # Retrieve and verify
            retrieved = session.query(WorkflowStep).filter_by(
                step_id='step_test'
            ).first()
