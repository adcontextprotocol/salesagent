...
def test_workflow_architecture():
    ...
    from models import Context, WorkflowStep, ObjectWorkflowMapping, MediaBuy
    ...
    try:
        ...
        session.query(WorkflowStep).filter(
            WorkflowStep.context_id.like('ctx_%')
        ).delete()
        session.query(Context).filter_by(
            tenant_id=tenant_id,
        ...
        table.add_row(
            "Context",
            "Track async conversations",
            "Simplified - no status/expires"
        )
        table.add_row(
            "WorkflowStep",
            "Work queue for tasks",
            "Added comments, removed started_at"
        )
        table.add_row(
