...
(``src.core.security.url_validator``) still EXISTS; it no longer carries the
SSRF half (salesagent-tbrk.1 / GH #1802). ``check_url_ssrf``,
``check_url_syntax``, ``BLOCKED_NETWORKS`` and ``BLOCKED_HOSTNAMES`` are the
symbols that are gone; what stays is the reserved-TLD family
(``RESERVED_TLDS`` / ``reserved_tld_for_host`` / ``is_reserved_tld_host``),
graded by ``TestReservedTldPolicy`` below.
...
Covers:
- reserved-TLD policy: the surviving half of ``src.core.security.url_validator``
...
class TestReservedTldPolicy:
    """The half of ``url_validator`` that SURVIVED #1802 still refuses.
