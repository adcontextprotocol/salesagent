...
``e2e_admin`` is named in the ``bdd_e2e`` env's ``-k "e2e_rest or e2e_admin"``.
Renaming either value without updating tox.ini would leave the scenarios
collected by no env — dormant, with CI still green.
"""
