...
class TestGetProductsRequestAlignment:
    ...
    def test_minimal_required_fields(self):
        ...
        req = GetProductsRequest(brand_manifest="Nike Air Jordan 2025 basketball shoes")

        assert req.brand_manifest == "Nike Air Jordan 2025 basketball shoes"
    ...
    def test_with_all_optional_fields(self):
        ...
        req = GetProductsRequest(
            brand_manifest="Acme Corp enterprise software",
        ...
        assert req.brand_manifest == "Acme Corp enterprise software"
    ...
    def test_filters_as_dict(self):
        ...
        req = GetProductsRequest(
            brand_manifest="Tesla Model Y electric vehicle",
            filters={
    ...
    def test_partial_filters(self):
        ...
        req = GetProductsRequest(
            brand_manifest="Spotify Premium music streaming", filters=ProductFilters(delivery_type="guaranteed")
        )
    ...
    def test_adcp_version_validation(self):
        ...
        for version in valid_versions:
            req = GetProductsRequest(brand_manifest="Test product", adcp_version=version)
            assert req.adcp_version == version

        # Invalid version format (should fail validation)
        with pytest.raises(ValidationError, match="pattern"):
            GetProductsRequest(brand_manifest="Test product", adcp_version="1.0")  # Missing patch version
    ...
    def test_filters_format_types_enum(self):
        ...
        for format_type in valid_types:
            req = GetProductsRequest(
                brand_manifest="Test product", filters=ProductFilters(format_types=[format_type])
            )
    ...
    def test_filters_delivery_type_values(self):
        ...
        # Guaranteed products
        req1 = GetProductsRequest(brand_manifest="Test product", filters=ProductFilters(delivery_type="guaranteed"))
        assert req1.filters.delivery_type == "guaranteed"

        # Non-guaranteed products
        req2 = GetProductsRequest(
            brand_manifest="Test product", filters=ProductFilters(delivery_type="non_guaranteed")
        )
...
class TestAdCPSchemaCompatibility:
    ...
    def test_example_from_adcp_spec_1(self):
        ...
        req = GetProductsRequest(
            brand_manifest="mobile apps", filters={"format_types": ["video"], "is_fixed_price": True}
        )

        assert req.brand_manifest == "mobile apps"
        assert req.filters.format_types == ["video"]
        assert req.filters.is_fixed_price is True

    def test_example_minimal_adcp_request(self):
        """Test minimal valid request per AdCP spec."""
        req = GetProductsRequest(brand_manifest="eco-friendly products")

        assert req.brand_manifest == "eco-friendly products"
        assert req.brief == ""
    ...
    def test_example_with_brief(self):
        ...
        req = GetProductsRequest(brief="display advertising", brand_manifest="eco-friendly products")
    ...
    def test_example_multiple_filter_fields(self):
        ...
        req = GetProductsRequest(
            brand_manifest="premium video content",
            filters={
                "delivery_type": "non_guaranteed",
                "format_types": ["video"],
...
class TestRegressionPrevention:
    ...
    def test_client_can_send_filters(self):
        ...
        try:
            ...
            assert req.brand_manifest == "cat food"
            assert req.adcp_version == "1.6.0"
            assert req.filters is not None
    ...
    def test_client_can_send_adcp_version(self):
        ...
        req = GetProductsRequest(brand_manifest="test product", adcp_version="1.6.0")
        assert req.adcp_version == "1.6.0"
    ...
    def test_wonderstruck_exact_payload(self):
        ...
        payload = {
            "brand_manifest": "purina cat food",
        ...
        assert req.brand_manifest == "purina cat food"
