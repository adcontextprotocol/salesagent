...
from tests.fixtures.builders import TestDataBuilder, TargetingBuilder
...
class FixtureGenerator:
    ...
    def generate_complete_scenario(self, name: str = "Test Scenario") -> dict:
        ...
        products = [
            ...
            ProductFactory.create_video_product(
                tenant_id=tenant["tenant_id"],
                name="Video Product"
            ),
            ProductFactory.create(
    ...
    def generate_performance_dataset(self, size: str = "medium") -> dict:
        ...
        for tenant in tenants:
            tenant_principals = PrincipalFactory.create_batch(
                principals_per_tenant,
                tenant_id=tenant["tenant_id"]
            )
        ...
        for tenant in tenants:
            tenant_products = ProductFactory.create_batch(
                products_per_tenant,
                tenant_id=tenant["tenant_id"]
            )
    ...
    def generate_edge_cases(self) -> dict:
        ...
        # Empty/minimal data
        cases["empty_tenant"] = TenantFactory.create(
            config={},
            name="",
            subdomain=""
        )
        ...
        # Extreme values
        cases["extreme_budget_buy"] = MediaBuyFactory.create(
            total_budget=0.01,  # Minimum
            spent_amount=0.01
        )
        
        cases["huge_budget_buy"] = MediaBuyFactory.create(
            total_budget=999999999.99,  # Maximum
            spent_amount=500000000.00
        )
    ...
    def generate_targeting_samples(self) -> dict:
        ...
        samples = {
            ...
            "demographic_focused": TargetingBuilder().with_demographics(
                age_ranges=["18-24", "25-34", "35-44"],
                genders=["male", "female"],
                income_ranges=["50k-75k", "75k-100k", "100k+"]
            ).build(),
            "device_specific": TargetingBuilder().with_devices(
                types=["mobile", "tablet"],
                os=["ios", "android"],
                browsers=["safari", "chrome"]
            ).build(),
            "content_targeted": TargetingBuilder().with_content(
                categories=["news", "sports", "entertainment"],
                keywords=["olympics", "football", "basketball"],
                topics=["sports", "fitness", "health"]
            ).build(),
            "signal_based": TargetingBuilder().with_signals([
...
def main():
    ...
    parser.add_argument(
        "--type",
        choices=["tenant", "principal", "product", "media_buy", "creative", "scenario", "performance", "edge_cases", "targeting"],
    ...
    parser.add_argument(
        ...
        default="medium",
        help="Size for performance dataset"
    )
