...
class TestMCPToolRoundtripMinimal:
    ...
    async def test_get_products_minimal(self, mcp_client):
        """Test get_products with only required parameter (brand_manifest)."""
        result = await mcp_client.call_tool("get_products", {"brand_manifest": "sustainable products"})
    ...
    async def test_create_media_buy_minimal(self, mcp_client):
        ...
        # Get a product first
        products = await mcp_client.call_tool("get_products", {"brand_manifest": "test product", "brief": "test"})
...
class TestSchemaConstructionValidation:
    ...
    def test_all_request_schemas_have_optional_or_default_fields(self):
        ...
        test_cases = [
            (schemas.GetProductsRequest, {"brand_manifest": "test"}),
