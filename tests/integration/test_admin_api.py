...
def test_superadmin_api(base_url=None):
    ...
    try:
        resp = requests.post(f"{base_url}/api/v1/superadmin/tenants", 
                            headers=headers, 
                            json=tenant_data)
        if resp.status_code == 201:
    ...
    try:
        resp = requests.get(f"{base_url}/api/v1/superadmin/tenants/{tenant_id}", 
                          headers=headers)
        if resp.status_code == 200:
            ...
            if 'adapter_config' in tenant_details:
                print(f"  Adapter configured: Yes")
    ...
    update_data = {
        ...
        "adapter_config": {
            "gam_company_id": "test_company_123",
            "gam_trafficker_id": "test_trafficker_456"
        }
    }
    
    try:
        resp = requests.put(f"{base_url}/api/v1/superadmin/tenants/{tenant_id}", 
                          headers=headers,
                          json=update_data)
        if resp.status_code == 200:
