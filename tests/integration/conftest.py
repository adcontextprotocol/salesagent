...
def populated_db(integration_db):
    ...
    """, (tenant["tenant_id"], tenant["name"], tenant["subdomain"], 
          tenant["config"], tenant["is_active"]))
    
    conn.execute("""
        INSERT INTO principals (tenant_id, principal_id, name, access_token, platform_mappings)
        VALUES (?, ?, ?, ?, ?)
    """, (principal["tenant_id"], principal["principal_id"], principal["name"],
          principal["access_token"], principal["platform_mappings"]))
    
    for product in products:
        conn.execute("""
            INSERT INTO products (tenant_id, product_id, name, formats, targeting_template)
            VALUES (?, ?, ?, ?, ?)
        """, (product["tenant_id"], product["product_id"], product["name"],
              product["formats"], product["targeting_template"]))
...
def test_media_buy_workflow(populated_db):
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (media_buy["tenant_id"], media_buy["media_buy_id"], media_buy["principal_id"],
          media_buy["status"], media_buy["config"], media_buy["total_budget"]))
    
    for creative in creatives:
        conn.execute("""
            INSERT INTO creatives (tenant_id, creative_id, principal_id, format_id, status, content)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (creative["tenant_id"], creative["creative_id"], creative["principal_id"],
              creative["format_id"], creative["status"], creative["content"]))
