    ...
    async with client:
        ...
        try:
            result = await client.call_tool('get_products', {
                'req': {
                    'brief': 'Test campaign for workflow testing'
                }
            })
        ...
        try:
            ...
            result = await client.call_tool('get_pending_workflows', {
                'req': {}
            })
            ...
            if hasattr(result, 'tasks') and result.tasks:
                ...
                for task in result.tasks[:3]:  # Show first 3
                    console.print(f"  - {task.get('task_id', 'N/A')}: {task.get('task_type', 'N/A')} ({task.get('status', 'N/A')})")
            else:
