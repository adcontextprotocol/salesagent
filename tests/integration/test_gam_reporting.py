...
def mock_gam_client():
    ...
    with (
        patch("gam_helper.get_db_session") as mock_gam_db,
...
class TestGAMReportingAPI:
    ...
    def test_get_reporting_requires_date_range(
        self, authenticated_session, mock_db_connection, mock_gam_client, mock_gam_service
    ):
    ...
    def test_get_reporting_validates_date_range(
        self, authenticated_session, mock_db_connection, mock_gam_client, mock_gam_service
    ):
    ...
    def test_get_reporting_validates_advertiser_id(
        self, authenticated_session, mock_db_connection, mock_gam_client, mock_gam_service
    ):
        """Test that invalid advertiser_id format is rejected."""
        response = authenticated_session.get(
            "/api/tenant/test_tenant/gam/reporting?date_range=today&advertiser_id=not-a-number"
        )
        assert response.status_code == 400
        data = json.loads(response.data)
        assert "error" in data
    ...
    def test_get_reporting_validates_timezone(
        self, authenticated_session, mock_db_connection, mock_gam_client, mock_gam_service
    ):
        """Test that invalid timezone is rejected."""
        response = authenticated_session.get(
            "/api/tenant/test_tenant/gam/reporting?date_range=today&timezone=Invalid/Timezone"
        )
        assert response.status_code == 400
        data = json.loads(response.data)
        assert "error" in data
