...
def complete_campaign_setup(live_server):
    ...
    response = requests.post(
        ...
        json={
            ...
            "platform_mappings": {"mock": {"advertiser_id": "test_adv_123"}}
        }
    )
    ...
    return {
        ...
        "auth_token": principal_data.get("access_token")
    }
        ...
        def report(self):
            ...
            if self.metrics:
                ...
                for metric in self.metrics:
                    print(f"  {metric['name']}: {metric['time']:.2f}s (CPU: {metric['cpu']:.1f}%, Mem: {metric['memory']:.1f}%)")
