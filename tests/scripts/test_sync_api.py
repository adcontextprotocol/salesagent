...
def test_sync_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if status_data.get("summary"):
                ...
                print(
                    f"   Summary: {summary.get('ad_units', 0)} ad units, "
    ...
    try:
        response = requests.post(
            trigger_url, headers=headers, json={"sync_type": "incremental"}  # Use incremental for faster testing
        )
    ...
    for sync in syncs:
        ...
        if sync["summary"]:
            ...
            print(
                f"   Results: {summary.get('ad_units', 0)} ad units, " f"{summary.get('custom_targeting_keys', 0)} keys"
            )
