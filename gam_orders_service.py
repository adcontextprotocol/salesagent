...
from models import GAMOrder, GAMLineItem, Tenant
from adapters.gam_orders_discovery import (
    GAMOrdersDiscovery, Order, LineItem, OrderStatus, LineItemStatus
)
...
class GAMOrdersService:
    ...
    def _upsert_order(self, tenant_id: str, order: Order, sync_time: datetime):
        ...
        existing = self.db.query(GAMOrder).filter_by(
            tenant_id=tenant_id,
    ...
    def _upsert_line_item(self, tenant_id: str, line_item: LineItem, sync_time: datetime):
        ...
        existing = self.db.query(GAMLineItem).filter_by(
            tenant_id=tenant_id,
    ...
    def get_orders(self, tenant_id: str, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        ...
        query = self.db.query(GAMOrder).options(
            joinedload(GAMOrder.line_items)
    ...
    def get_line_items(self, tenant_id: str, order_id: Optional[str] = None, 
    ...
    def get_order_details(self, tenant_id: str, order_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        order = self.db.query(GAMOrder).filter_by(
            tenant_id=tenant_id,
            order_id=order_id
        ).first()
        ...
        line_items = self.db.query(GAMLineItem).filter_by(
            tenant_id=tenant_id,
            order_id=order_id
        ).all()
        ...
        result['stats'] = {
            ...
            'total_spend': sum((li.cost_per_unit or 0) * (li.stats_impressions or 0) / 1000 for li in line_items if li.cost_type == 'CPM')
        }
            ...
            for li in line_items:
                if li.status == 'APPROVED':
                    if li.start_date and li.end_date:
                        ...
                        start_dt = dt.datetime.combine(li.start_date, dt.datetime.min.time(), timezone.utc) if isinstance(li.start_date, date) else li.start_date
                        end_dt = dt.datetime.combine(li.end_date, dt.datetime.max.time(), timezone.utc) if isinstance(li.end_date, date) else li.end_date
    ...
    def _line_item_to_dict(self, line_item: GAMLineItem) -> Dict[str, Any]:
        ...
        delivery_percentage = 0.0
        if line_item.primary_goal_type == 'IMPRESSIONS' and line_item.primary_goal_units and line_item.stats_impressions:
